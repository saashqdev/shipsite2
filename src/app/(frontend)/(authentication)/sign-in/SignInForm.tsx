    ...
    <div className="grow flex flex-col items-center justify-center p-4 bg-slate-50 py-32">
      {media && (
        <Link href="/" className="mb-8">
          <Media resource={media} className="w-32 mx-auto" />
        </Link>
      )}
      <Card className="w-full max-w-md bg-white shadow-xl">
        ...
        <CardContent>
          {authMethods.includes('credentials') || authMethods.includes('magicLink') ? (
            <Form {...form}>
              ...
              >
                ...
                {((showPasswordField && authMethods.includes('credentials')) ||
                  (authMethods.includes('credentials') && !authMethods.includes('magicLink'))) && (
                  <FormField
                    control={form.control}
                    name="password"
                ...
                {!showPasswordField && authMethods.includes('magicLink') && (
                  <LoadingButton
                    pending={!!pending.magicLink}
                    onClick={() => handleMagicLinkSignIn(form.getValues().email)}
                  >
                    Send Magic Link
                  </LoadingButton>
                )}
          ...
          {authMethods
            .filter((method): method is (typeof validProviders)[number] =>
              validProviders.includes(method as any),
